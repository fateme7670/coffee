"use client";
import { AnswerTiketFromServer } from "@/Redux/features/Tiket";
import { BanUserFromServer } from "@/Redux/features/users";
...
const TableTikets = ({ tickets }: Props) => {
  ...
  const banedUser = async (email: string, phone: string) => {
    swal({
      ...
      if (result) {
        dispatch(BanUserFromServer({ email, phone })).then((data) => {
  ...
  const sendanswer = async (ticket: TiketProps) => {
    swal({
      ...
      dispatch(AnswerTiketFromServer({ awnser })).then((data) => {
        if (data?.payload?.message === "success") {
          return showSwal("tiket answerd sucessfully", "success", "Ok");
        }
      });
              ...
              <tr key={tiket._id}>
                ...
                <td className="flex items-center gap-2 justify-center">
                  ...
                  <AiOutlineStop
                    ...
                    onClick={() =>
                      banedUser(tiket.user?.email, tiket.user?.phone)
                    }
                  />
                </td>
